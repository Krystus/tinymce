...
const wrapInFigure = (image: HTMLElement) => {
  ...
  figureElm.appendChild(DOM.create('figcaption', { contentEditable: 'true' }, tinymce.translate('Caption')));
...
const create = (normalizeCss: CssNormalizer, data: ImageData): HTMLElement => {
  ...
  if (data.caption) {
    ...
    figure.appendChild(DOM.create('figcaption', { contentEditable: 'true' }, tinymce.translate('Caption')));
